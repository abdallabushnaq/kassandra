...
//    }
    ...
    private void drawCriticalMarker(Task task, int x1, int x2, int y) {
        if (task.getCritical()) {
            graphics2D.setColor(theme.ganttTheme.criticalTaskBorderColor);
        } else {
            graphics2D.setColor(theme.ganttTheme.taskBorderColor);
        }
        ProjectCalendar pc;
    ...
    private void drawId(Task task, int y) {
        ...
        Color    fillColor = theme.ganttTheme.idBgColor;
        Color    textColor = theme.ganttTheme.idTextColor;
        MetaData md        = TaskUtil.getTaskMetaData(task);
        graphics2D.setFont(idFont);
        fillColor = theme.ganttTheme.idBgColor;
        textColor = theme.ganttTheme.idTextColor;
...
//    }
    ...
    private void drawRelation(Task sourceTask, int y2, Task targetTask, int y1) throws Exception {
        ...
        if (sourceTask.getCritical() && targetTask.getCritical()) {
            graphics2D.setColor(theme.ganttTheme.criticalRelationColor);
...
//            }
    ...
    private void drawTask(Task task, int x1, int x2, int y, boolean labelInside, boolean alien, String marker, List<Conflict> conflict) throws Exception {
        Color  fillColor           = theme.burndownTheme.getAuthorColor(28);
        Color  textColor           = theme.ganttTheme.taskTextColor;
        Color  textInfoColor       = theme.ganttTheme.taskTextColor;
        String resourceName        = null;
        ...
        if (task.isMilestone() && task.getChildTasks().size() == 0) {
            ...
            textColor = theme.ganttTheme.milestoneTextColor;
        } else if (task.getChildTasks().size() != 0) {
            // ---Story
            fillColor = theme.ganttTheme.storyColor;
            textColor = theme.ganttTheme.storyTextColor;
        } else if (resourceName != null && units != null) {
            //task
            textColor = theme.ganttTheme.taskTextColor;
...
//                    graphics2D.fillRect(x1 + 1, y1 + 4, (int) ((x2 - x1) * progress - 1), h - 8);
    ...
    private void drawTick(LocalDateTime time, int x, int y, TextAlignment alignment) {
        ...
        graphics2D.setColor(theme.ganttTheme.taskTickLineColor);
        graphics2D.fillRect(x, y - getTaskHeight() / 2 + TASK_BODY_BORDER - 2, 1, 2);
        graphics2D.setFont(taskTickFont);
        graphics2D.setColor(theme.ganttTheme.taskTickTextColor);
        FontMetrics fm    = graphics2D.getFontMetrics();
    ...
    private Color generateGanttColor(Color color) {
        ...
        return new Color(color.getRed(), color.getGreen(), color.getBlue(), theme.ganttTheme.taskTransparency);
    }

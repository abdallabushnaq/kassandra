...
(function () {
    ...
    class AbstractGanttRenderer extends window.AbstractRenderer {
        constructor(theme, milestones, preRun, postRun) {
            ...
            this.chartStart = null;  // local midnight of chart first day
            this.totalDays = 0;
        ...
        drawStoryBody(g, task, x1, x2, y, marker) {
            ...
            if (marker == null) {
                ...
                if (x2 - x1 - 1 > 0) {
                    var tooltipRect = createRect(x1 + 1, y1 - this.getTaskHeight() / 2, x2 - x1 - 1, this.getTaskHeight() - thickness * 2, {
                        fill: 'none',
                        'pointer-events': 'all'
                    });
    ...
    class GanttRenderer extends AbstractGanttRenderer {
        ...
        constructor(data, theme) {
            ...
            // Mirrors Java GanttRenderer constructor which creates milestones from sprint dates
            var chartStart = getDayMidnight(new Date(data.meta.chartStart));
            var chartEnd = getDayMidnight(new Date(data.meta.chartEnd));
            var now = data.meta.now ? getDayMidnight(new Date(data.meta.now)) : getDayMidnight(new Date());
            var earliestStart = getDayMidnight(new Date(data.meta.sprintEarliestStartDate));
            var latestFinish = getDayMidnight(new Date(data.meta.sprintLatestFinishDate));
            var sprintStatus = data.meta.sprintStatus;

            var milestonesList = [];

            // Milestone "N" (Now) - current date (unless sprint is CLOSED)
            ...
            // Milestone "S" (Start) - earliest start date
            milestonesList.push(new window.Milestone(
                earliestStart,
                'S',
            ...
            // Milestone "E" (End) - latest finish date
            milestonesList.push(new window.Milestone(
                latestFinish,
                'E',
                'End (End of project)',
            ...
            var milestones = new window.Milestones(
                milestonesList,
            ...
            this.totalDays = calculateDayCount(
                getDayMidnight(new Date(data.meta.chartStart)),
                getDayMidnight(new Date(data.meta.chartEnd))
            );

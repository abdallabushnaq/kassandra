...
public class ErDiagramRenderer {
    ...
    /**
     * Draws a footer band at the bottom of the canvas showing the generator
     * name ("Generated by Kassandra") and the current date and time.
     *
     * @param g            graphics context
     * @param canvasWidth  total canvas width in pixels
     * @param canvasHeight total canvas height in pixels (footer is at the bottom)
     */
    protected void drawFooter(Graphics2D g, int canvasWidth, int canvasHeight) {
        int footerY = canvasHeight - FOOTER_HEIGHT;

        // Background band
        g.setColor(FOOTER_BG);
        g.fillRect(0, footerY, canvasWidth, FOOTER_HEIGHT);

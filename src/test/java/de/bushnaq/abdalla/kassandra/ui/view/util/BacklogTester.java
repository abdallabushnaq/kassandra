...
@Lazy
@Log4j2
public class BacklogTester extends AbstractViewTester {
     ...
     */
    public BacklogTester(HumanizedSeleniumHandler seleniumHandler, @Value("${local.server.port:8080}") int port) {
    ...
    public void switchToBacklog() {
        seleniumHandler.click("/" + Backlog.ROUTE);
        seleniumHandler.waitForUrl("http://localhost:" + port + "/ui/" + Backlog.ROUTE);
    }
}
